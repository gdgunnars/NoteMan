...
import { fetchNotes, search, getFilesForId, removeNote, saveModel,
    findById } from "../data_access/noteData";
 ...
 */
noteRouter.post("/", upload.array("files", 5), async (req, res) => {
    try {
        const files = [];
 ...
 */
noteRouter.put("/:id", upload.array("files", 5), async (req, res) => {
    try {
        if(!req.params.id || !mongoose.Types.ObjectId.isValid(req.params.id)){
            return res.status(400).json({message: "ID invalid or not on correct format"})
        }
        ...
        if (req.files && req.files.length > 0) {
            const newFiles = await createFileListFromModel(req.files)
            files.push(...newFiles);
            await Object.assign(note, {files: files});
        }
